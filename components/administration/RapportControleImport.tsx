...
export function RapportControleImport({ rapport }: { rapport: RapportImport }) {
  ...
  /* Document 16, C.2 : lignes importees mais signalees, une salle sans capacite par exemple. */
  const lignesAvertissement = rapport.lignesValides.filter((l) => l.avertissements.length > 0);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-4" style={{ fontSize: "var(--text-body)" }}>
        ...
        {lignesAvertissement.length > 0 && (
          <span style={{ color: "var(--color-text-secondary)", fontWeight: 600 }}>
            {t("admin.import.rapport.lignes_avertissement", { n: lignesAvertissement.length })}
          </span>
        )}
      </div>
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid var(--color-border-strong)" }}>
              <th className="px-2 py-1.5 text-left" style={{ color: "var(--color-text-secondary)" }}>
                {t("admin.import.rapport.colonne_ligne")}
              </th>
              <th className="px-2 py-1.5 text-left" style={{ color: "var(--color-text-secondary)" }}>
      ...
      {lignesAvertissement.length > 0 && (
        <div className="flex flex-col gap-2">
          <h4
            style={{ fontFamily: "var(--font-titre)", fontSize: "var(--text-body)", fontWeight: 600 }}
          >
            {t("admin.import.rapport.avertissements")}
          </h4>
          <table className="w-full" style={{ fontSize: "var(--text-small)" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--color-border-strong)" }}>
            ...
            <tbody>
              {lignesAvertissement.map((ligne) => (
                <tr key={ligne.numeroLigne} style={{ borderBottom: "1px solid var(--color-border)" }}>
                  <td className="px-2 py-1.5" style={{ color: "var(--color-text-secondary)", fontWeight: 600 }}>
                    {t("admin.import.rapport.ligne", { n: ligne.numeroLigne })}
                  </td>
                  <td className="px-2 py-1.5">
                    {ligne.avertissements.map((m, i) => (
                      <div key={i}>{t(m.cle, m.variables)}</div>
                    ))}
                  </td>
                </tr>
